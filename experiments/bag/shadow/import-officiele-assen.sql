...
BEGIN
  ...
  IF v_bag_rows <> 0 THEN
    RAISE EXCEPTION 'Officiële Assen-import vereist een lege BAG-shadow; rijen=%', v_bag_rows;
  END IF;
...
INSERT INTO bag_control.datasetversies (
  ...
  'v20200601-officiele-assen-shadow',
  '0106',
  'staging',
  false,
  :'bron_checksum',
  jsonb_build_object(
    'officieel', true,
    'bron', 'Kadaster Proefbestand gemeente Assen',
    'gemeentecode', '0106',
...
SELECT
  ...
  jsonb_build_object('bron', 'officiele_assen_export')
...
SELECT 1 / (
  (SELECT count(*) FROM bag_staging.objecten WHERE datasetversie_id = :datasetversie_id)
    = :expected_objecten::bigint
  AND (SELECT count(*) FROM bag_staging.voorkomens WHERE datasetversie_id = :datasetversie_id)
    = :expected_voorkomens::bigint
  AND (SELECT count(*) FROM bag_staging.relaties WHERE datasetversie_id = :datasetversie_id)
    = :expected_relaties::bigint
  AND (
    (SELECT count(*) FROM bag_staging.geometrieen WHERE datasetversie_id = :datasetversie_id)
    + (SELECT count(*) FROM bag_control.geometrie_afwijkingen WHERE datasetversie_id = :datasetversie_id)
  ) = :expected_geometrieen::bigint
...
SELECT * FROM bag_staging.objecten WHERE datasetversie_id = :datasetversie_id;
INSERT INTO bag_published.voorkomens
SELECT * FROM bag_staging.voorkomens WHERE datasetversie_id = :datasetversie_id;
INSERT INTO bag_published.relaties
SELECT * FROM bag_staging.relaties WHERE datasetversie_id = :datasetversie_id;
INSERT INTO bag_published.geometrieen
SELECT * FROM bag_staging.geometrieen WHERE datasetversie_id = :datasetversie_id;
...
SELECT 'OFFICIELE_ASSEN_SHADOW_IMPORT_OK' AS status, :datasetversie_id::bigint AS datasetversie_id;
