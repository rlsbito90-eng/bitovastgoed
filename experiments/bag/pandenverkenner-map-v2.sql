...
AS $function$
DECLARE
  v_grid_m double precision;
BEGIN
  IF p_scope_code IS NULL OR NOT (p_scope_code ~ '^[0-9]{4}$') THEN RAISE EXCEPTION 'Ongeldige BAG-scopecode'; END IF;
  IF p_limiet < 1 OR p_limiet > 1500 THEN RAISE EXCEPTION 'Kaartlimiet moet tussen 1 en 1500 liggen'; END IF;
  IF p_zoom < 7 OR p_zoom > 22 THEN RAISE EXCEPTION 'Kaartzoom moet tussen 7 en 22 liggen'; END IF;
  ...
  END IF;
  IF p_vbo_modus NOT IN ('alle','met_vbo','zonder_vbo') THEN RAISE EXCEPTION 'Ongeldige VBO-modus'; END IF;
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM unnest(COALESCE(p_wijk_codes, ARRAY[]::text[])) value WHERE NOT (value ~ '^WK[0-9]{4}[A-Z0-9]{2}$') OR substr(value,3,4)<>p_scope_code)
     OR EXISTS (SELECT 1 FROM unnest(COALESCE(p_buurt_codes, ARRAY[]::text[])) value WHERE NOT (value ~ '^BU[0-9]{4}[A-Z0-9]{4}$') OR substr(value,3,4)<>p_scope_code) THEN
  ...
  v_grid_m := CASE
    WHEN p_zoom < 10 THEN 4000
    WHEN p_zoom < 11 THEN 2500
    WHEN p_zoom < 12 THEN 1500
    WHEN p_zoom < 13 THEN 1000
    WHEN p_zoom < 14 THEN 500
    ELSE 250
  END;
  ...
  IF p_zoom < 15 THEN
    ...
    WITH actieve AS MATERIALIZED (
      ...
      JOIN bag_control.datasetversies d ON d.id=b.datasetversie_id AND d.scope_code=b.scope_code AND d.status='actief' AND d.is_actief
      WHERE b.scope_code=p_scope_code AND b.status='actief' AND b.validatie_fouten=0 AND b.gebouwd_panden=b.verwacht_panden
    ), viewport AS MATERIALIZED (
      SELECT extensions.st_transform(extensions.st_makeenvelope(p_min_lon,p_min_lat,p_max_lon,p_max_lat,4326),28992) AS geom
    ), gefilterd AS MATERIALIZED (
      SELECT i.centroid
      ...
      WHERE i.scope_code=p_scope_code AND i.centroid IS NOT NULL
        AND i.centroid && v.geom AND extensions.st_intersects(i.centroid,v.geom)
      ...
      SELECT
        floor(extensions.st_x(centroid)/v_grid_m)::bigint AS gx,
        floor(extensions.st_y(centroid)/v_grid_m)::bigint AS gy,
        count(*)::integer AS cnt,
        avg(extensions.st_x(centroid)) AS cx,
        avg(extensions.st_y(centroid)) AS cy
      FROM gefilterd
      GROUP BY 1,2
    )
    SELECT
      'cluster'::text,
      concat(v_grid_m::integer,':',gx,':',gy),
      cnt,
      NULL::bigint,NULL::bigint,NULL::text,NULL::text,NULL::integer,NULL::integer,NULL::numeric,NULL::text[],NULL::boolean,
      NULL::text,NULL::text,NULL::text,NULL::text,NULL::text,NULL::text,NULL::text,
      extensions.st_asgeojson(extensions.st_transform(extensions.st_setsrid(extensions.st_makepoint(cx,cy),28992),4326),6,0)::jsonb,
      NULL::jsonb,
      false
    FROM clusters
    ORDER BY cnt DESC, gx, gy;
    RETURN;
  END IF;
  ...
  WITH actieve AS MATERIALIZED (
    ...
    SELECT t.*, count(*) OVER () > p_limiet AS is_afgekapt FROM treffers t
  )
  SELECT
    'pand'::text,NULL::text,1,
