-- Pandenverkenner 2.0 BUILD 1C.2 — kandidaatfunctie, NIET automatisch migreren.
-- Doel: multiselect voor BAG-pandstatus en gebruiksfunctie bovenop de actieve gevalideerde index.
-- Semantiek: OR binnen statussen; OR binnen gebruiksdoelen; AND tussen filtergroepen.
...
CREATE OR REPLACE FUNCTION bag_service.zoek_panden_v3(
  ...
  p_statussen text[] DEFAULT ARRAY[]::text[],
  ...
  p_gebruiksdoelen text[] DEFAULT ARRAY[]::text[],
...
BEGIN
  ...
  END IF;
  IF cardinality(p_statussen) > 16 OR cardinality(p_gebruiksdoelen) > 16 THEN
    RAISE EXCEPTION 'Te veel multiselectopties';
  END IF;
  IF EXISTS (SELECT 1 FROM unnest(p_statussen) s WHERE s IS NULL OR btrim(s)='' OR length(s)>128)
     OR EXISTS (SELECT 1 FROM unnest(p_gebruiksdoelen) g WHERE g IS NULL OR btrim(g)='' OR length(g)>128) THEN
  ...
  SELECT
    i.datasetversie_id, i.index_build_id, i.pand_identificatie, i.voorkomen_sleutel,
    i.pandstatus_huidig, i.oorspronkelijk_bouwjaar, i.heeft_vbo, i.vbo_aantal,
    i.vbo_oppervlakte_som, i.vbo_oppervlakte_max, i.gebruiksdoelen, i.is_gemengd,
    i.primair_nummeraanduiding_id, i.primair_adres, i.primair_straat, i.primair_huisnummer,
    i.primair_postcode, i.primair_plaats, i.adres_count,
    i.gemeente_code, i.gemeente_naam, i.wijk_code, i.wijk_naam, i.buurt_code, i.buurt_naam,
    i.centroid, i.pand_geometrie, i.pand_identificatie AS volgende_cursor
  ...
  WHERE (p_na_identificatie IS NULL OR i.pand_identificatie > p_na_identificatie)
    ...
    AND (cardinality(p_statussen)=0 OR i.pandstatus_huidig = ANY(p_statussen))
    ...
    AND (cardinality(p_gebruiksdoelen)=0 OR i.gebruiksdoelen && p_gebruiksdoelen)
...
-- Bewust geen tabel-GRANTs. Alleen execute via de bestaande bag_reader-transportgrens na expliciete apply-gate.
