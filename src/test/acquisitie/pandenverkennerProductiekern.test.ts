...
describe('Pandenverkenner productiekernbron',()=>{
  ...
  it('weigert twee of nul dossierbronnen',()=>{expect(valideerBriefcontract({...basis,signaalId:'sig',vastgoedkansId:'k1'})).toContain(expect.stringMatching(/Exact één dossierbron/));expect(valideerBriefcontract({...basis,signaalId:null,vastgoedkansId:null})).toContain(expect.stringMatching(/Exact één dossierbron/))});
