...
describe('post copyvarianten', () => {
  it('bouwt de goedgekeurde splitsingspotentie Brief 1 challenger B', () => {
    const template = bouwPostVariantTemplate({ ...basis, toewijzing: { profiel: 'splitsingspotentie', variantKey: 'splitsingspotentie:post:brief_1:B', variantCode: 'B' } });
  ...
  it('valt voor Brief 1 controle A terug op de bestaande standaardbrief met open opvolglogica', () => {
    const template = bouwPostVariantTemplate({ ...basis, toewijzing: { profiel: 'splitsingspotentie', variantKey: 'splitsingspotentie:post:brief_1:A', variantCode: 'A' } });
  ...
  it('bouwt voor Splitsingspotentie Brief 2 een echte follow-up als controle A', () => {
    const template = bouwPostVariantTemplate({ ...basis, toewijzing: { profiel: 'splitsingspotentie', variantKey: 'splitsingspotentie:post:brief_2:A', variantCode: 'A' } });
  ...
  it('bouwt de compacte Splitsingspotentie Brief 2 challenger B met rustige objectgerichte onderwerpregel', () => {
    const template = bouwPostVariantTemplate({ ...basis, toewijzing: { profiel: 'splitsingspotentie', variantKey: 'splitsingspotentie:post:brief_2:B', variantCode: 'B' } });
  ...
  it('bouwt Splitsingspotentie Brief 3 als gecontroleerde afsluiting met ruimte voor nurture', () => {
    const template = bouwPostVariantTemplate({ ...basis, toewijzing: { profiel: 'splitsingspotentie', variantKey: 'splitsingspotentie:post:brief_3:A', variantCode: 'A' } });
  ...
  it('bouwt de goedgekeurde compacte Splitsingspotentie Brief 3 challenger B', () => {
    const template = bouwPostVariantTemplate({ ...basis, toewijzing: { profiel: 'splitsingspotentie', variantKey: 'splitsingspotentie:post:brief_3:B', variantCode: 'B' } });
    expect(template.onderwerp).toBe('Uw pand aan Voorbeeldstraat 10 te Amsterdam');
    expect(template.brieftekst).toContain('Nog één keer kort over Voorbeeldstraat 10 te Amsterdam');
  ...
  it('bouwt Woonvorming Brief 1 controle A met expliciete context en relativering', () => {
    const template = bouwPostVariantTemplate({ ...basis, toewijzing: { profiel: 'woonvorming', variantKey: 'woonvorming:post:brief_1:A', variantCode: 'A' } });
  ...
  it('bouwt Woonvorming Brief 1 challenger B als kortere objectgerichte variant', () => {
    const template = bouwPostVariantTemplate({ ...basis, toewijzing: { profiel: 'woonvorming', variantKey: 'woonvorming:post:brief_1:B', variantCode: 'B' } });
    expect(template.onderwerp).toBe('Uw pand aan Voorbeeldstraat 10 te Amsterdam');
  ...
  it('bouwt Woonvorming Brief 2 controle A als herkenbare follow-up met korte context', () => {
    const template = bouwPostVariantTemplate({ ...basis, toewijzing: { profiel: 'woonvorming', variantKey: 'woonvorming:post:brief_2:A', variantCode: 'A' } });
  ...
  it('bouwt de goedgekeurde compacte Woonvorming Brief 2 challenger B', () => {
    const template = bouwPostVariantTemplate({ ...basis, toewijzing: { profiel: 'woonvorming', variantKey: 'woonvorming:post:brief_2:B', variantCode: 'B' } });
    expect(template.onderwerp).toBe('Uw pand aan Voorbeeldstraat 10 te Amsterdam');
    expect(template.brieftekst).toContain('Enige tijd geleden schreef ik u over Voorbeeldstraat 10 te Amsterdam');
  ...
  it('bouwt Woonvorming Brief 3 controle A als rustige sequence-afsluiting met context', () => {
    const template = bouwPostVariantTemplate({ ...basis, toewijzing: { profiel: 'woonvorming', variantKey: 'woonvorming:post:brief_3:A', variantCode: 'A' } });
    expect(template.onderwerp).toBe('Over uw pand aan Voorbeeldstraat 10 te Amsterdam');
  ...
  it('bouwt de goedgekeurde Woonvorming Brief 3 challenger B met natuurlijke opening', () => {
    const template = bouwPostVariantTemplate({ ...basis, toewijzing: { profiel: 'woonvorming', variantKey: 'woonvorming:post:brief_3:B', variantCode: 'B' } });
    expect(template.onderwerp).toBe('Uw pand aan Voorbeeldstraat 10 te Amsterdam');
