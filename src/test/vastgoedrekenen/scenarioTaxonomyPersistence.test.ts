...
describe('scenario-taxonomie Fase 2 — dual read en persistence', () => {
  ...
  it('geeft een volledig en gemarkeerd canoniek record voorrang op legacy', () => {
    const result = resolvePersistedScenarioTaxonomy({
      ...
      business_case: 'income_investment',
      intervention: 'none',
  ...
  it('maakt één volledige atomaire snake_case persistence patch', () => {
    expect(scenarioTaxonomyPersistencePatch({
  ...
  it('weigert in strict mode uitbreiden zonder uitbreidingstype', () => {
    expect(() => scenarioTaxonomyPersistencePatch({
      ...
      exploitation: 'rental',
      disposition: 'hold',
    })).toThrow(ScenarioTaxonomyPersistenceError);
  });

  it('reset uitsluitend via een expliciete volledige clear-patch naar legacy-read', () => {
    expect(clearScenarioTaxonomyPersistencePatch()).toEqual({
      business_case: null,
      intervention: null,
      expansion_subtype: null,
...
describe('Quickscan-metadata Fase 2', () => {
  it('maakt uitsluitend genormaliseerde metadata-kolommen', () => {
    expect(analysisMetadataPersistencePatch({
      analysisQuestion: '  Wat is de maximale aankoopprijs?  ',
      valuationDate: '2026-07-28',
      timeHorizonMonths: '36',
    })).toEqual({
      analysis_question: 'Wat is de maximale aankoopprijs?',
  ...
  it('weigert ongeldige peildata en tijdshorizonnen', () => {
    expect(() => analysisMetadataPersistencePatch({ valuationDate: '2026-02-30' }))
      .toThrow(AnalysisMetadataValidationError);
    expect(() => analysisMetadataPersistencePatch({ timeHorizonMonths: 0 }))
      .toThrow(AnalysisMetadataValidationError);
    expect(() => analysisMetadataPersistencePatch({ timeHorizonMonths: 1201 }))
      .toThrow(AnalysisMetadataValidationError);
  });

  it('leest oude of ongeldige metadata defensief zonder records te muteren', () => {
    const record = {
      analysis_question: 123,
      valuation_date: 'geen-datum',
    ...
    expect(result).toEqual({
      ...
      warnings: expect.arrayContaining([
        expect.stringContaining('analysevraag'),
        expect.stringContaining('peildatum'),
        expect.stringContaining('tijdshorizon'),
      ]),
    });
  });
});
