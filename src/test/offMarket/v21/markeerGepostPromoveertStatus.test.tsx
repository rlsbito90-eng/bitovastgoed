// V2.1 — markeer gepost promoveert signaalstatus naar 'benaderd'
// vanuit eerdere fases. Tests via supabase-mock op de useMarkBriefVerstuurd hook.
...
describe('useMarkBriefVerstuurd — statuspromotie', () => {
  it('eigenaar_gevonden → wordt benaderd', async () => {
  ...
  it('te_onderzoeken → wordt benaderd', async () => {
    mockSupabase('te_onderzoeken');
    const { result } = renderHook(() => useMarkBriefVerstuurd(), { wrapper: wrap() });
    await result.current.mutateAsync({ id: 'b1', postdatum: '2026-06-01' });
    expect(signaalUpdateMock).toHaveBeenCalledWith({ status: 'benaderd' });
  });
