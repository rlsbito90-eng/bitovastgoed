...
export default function VastgoedkansenPage() {
  ...
  const list = useMemo(
    () => kansen.filter((kans) => {
      ...
      if (!q) return true;
      const zoektekst = [kans.korteOmschrijving,kans.adres,kans.postcode,kans.plaats,kans.typeVastgoed,kans.redenInteressant,kans.eigenaarNaam].filter(Boolean).join(' ').toLowerCase();
    ...
    <div className="page-shell-wide min-w-0 overflow-x-hidden">
      ...
      <div className="flex max-w-full gap-2 overflow-x-auto pb-1">
        {STATUS_VOLGORDE.map((status) => (
          <button key={status} onClick={() => setWerkbak(status)} className={`shrink-0 rounded-full border px-3 py-1.5 text-xs ${werkbak === status ? 'bg-foreground text-background' : 'bg-card text-muted-foreground'}`}>
      ...
      <div className="relative max-w-xl min-w-0">
        ...
        <Input className="min-w-0 pl-9" value={q} onChange={(event) => setQ(event.target.value)} placeholder="Zoek adres, plaats, type, eigenaar of reden…" />
      </div>

      <section className="section-card min-w-0 overflow-hidden">
        {laden ? <p className="p-8 text-sm text-muted-foreground">Laden…</p> : list.length === 0 ? (
          <div className="p-10 text-center"><Database className="mx-auto h-8 w-8 text-muted-foreground/50" /><p className="mt-3 text-sm text-muted-foreground">Geen vastgoedkansen in deze werkbak.</p></div>
        ) : (
              ...
              <div key={kans.id} className="flex min-w-0 items-start gap-3 px-4 py-3 sm:px-5">
                <div className="min-w-0 flex-1">
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    <Link to={`/vastgoedkansen/${kans.id}`} className="min-w-0 break-words text-sm font-medium hover:text-primary hover:underline">{kansTitel(kans)}</Link>
                  ...
                  <p className="mt-1 flex flex-wrap gap-x-2 text-xs text-muted-foreground">
                    <span>{kans.typeVastgoed || 'Type onbekend'}</span><span>· {HERKOMST_LABEL[kans.herkomst]}</span><span>· Eigenaar: {kans.eigenaarStatus.replace('_', ' ')}</span><span>· Brief: {kans.briefStatus.replace('_', ' ')}</span>
                  </p>
