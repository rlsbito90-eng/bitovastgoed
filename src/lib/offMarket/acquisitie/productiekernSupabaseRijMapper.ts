...
function canoniekTijdstip(rij: Rij, veld: string, entiteit: string): string {
  const waarde = tekst(rij, veld, entiteit);
  if (!POSTGREST_TIMESTAMPTZ.test(waarde)) {
    throw new ProductiekernRijOngeldigError(entiteit, `${veld} is geen geldig timestamptz`);
  }
  const tijdMs = Date.parse(waarde);
  if (!Number.isFinite(tijdMs)) {
    throw new ProductiekernRijOngeldigError(entiteit, `${veld} is niet parseerbaar`);
  }
  return new Date(tijdMs).toISOString();
}
function nullableCanoniekTijdstip(rij: Rij, veld: string, entiteit: string): string | null {
  const waarde = rij[veld];
  if (waarde === null || waarde === undefined) return null;
  if (typeof waarde !== 'string') {
    throw new ProductiekernRijOngeldigError(entiteit, `${veld} is geen tekst`);
  }
  return canoniekTijdstip(rij, veld, entiteit);
}
...
export function mapAcquisitiedossierRij(rij: Rij): AcquisitiedossierContract {
  return {
    ...
    objectId: nullableTekst(rij, 'object_id', 'Acquisitiedossier'), verwerkingGestartOp: nullableCanoniekTijdstip(rij, 'verwerking_gestart_op', 'Acquisitiedossier'),
    ...
    volgendeActieOp: nullableCanoniekTijdstip(rij, 'volgende_actie_op', 'Acquisitiedossier'), volgendeActieOmschrijving: nullableTekst(rij, 'volgende_actie_omschrijving', 'Acquisitiedossier'),
  };
}
...
export function mapBriefRij(rij: Rij): BriefContract {
  return {
    ...
    vervangingVanBriefId: nullableTekst(rij, 'vervanging_van_brief_id', 'Brief'), definitiefOp: nullableCanoniekTijdstip(rij, 'definitief_op', 'Brief'),
    vergrendeldOp: nullableCanoniekTijdstip(rij, 'vergrendeld_op', 'Brief'), annuleringsreden: nullableTekst(rij, 'annuleringsreden', 'Brief'),
  };
}
...
export function mapBriefversieRij(rij: Rij): BriefversieContract {
  return {
    ...
    bestandReferentie: nullableTekst(rij, 'bestand_referentie', 'Briefversie'), createdAt: canoniekTijdstip(rij, 'created_at', 'Briefversie'),
    vervallenOp: nullableCanoniekTijdstip(rij, 'vervallen_op', 'Briefversie'), verzondenOp: nullableCanoniekTijdstip(rij, 'verzonden_op', 'Briefversie'),
  };
}
...
export function mapPrintbatchRij(rij: Rij): PrintbatchContract {
  return {
    ...
    printdatum: nullableCanoniekTijdstip(rij, 'printdatum', 'Printbatch'), verzenddatum: nullableCanoniekTijdstip(rij, 'verzenddatum', 'Printbatch'),
    geannuleerdOp: nullableCanoniekTijdstip(rij, 'geannuleerd_op', 'Printbatch'), annuleringsreden: nullableTekst(rij, 'annuleringsreden', 'Printbatch'),
  };
}
...
export function mapPrintbatchBriefRij(rij: Rij): PrintbatchBriefContract {
  return {
    ...
    briefVersieId: tekst(rij, 'brief_versie_id', 'Printbatchbrief'), verwijderdOp: nullableCanoniekTijdstip(rij, 'verwijderd_op', 'Printbatchbrief'),
...
export function mapBatchdocumentRij(rij: Rij): BatchdocumentContract {
  return {
    ...
    createdAt: canoniekTijdstip(rij, 'created_at', 'Batchdocument'),
    vervallenOp: nullableCanoniekTijdstip(rij, 'vervallen_op', 'Batchdocument'),
  };
}
