...
export interface PrintbatchBriefContract {
  id: string;
  batchId: string;
  briefId: string;
  ...
  afwijkingsreden: string | null;
}

export interface BatchdocumentContract {
  id: string;
  batchId: string;
  documentversie: number;
  documenttype: Batchdocumenttype;
  bestandReferentie: string;
...
const BATCH_OVERGANGEN: Record<Printbatchstatus, readonly Printbatchstatus[]> = {
  concept: ['documenten_gegenereerd', 'geannuleerd'],
  documenten_gegenereerd: ['concept', 'geprint', 'geannuleerd'],
  geprint: ['gedeeltelijk_gepost', 'gepost'],
  gedeeltelijk_gepost: ['gepost'],
  gepost: [],
  geannuleerd: [],
};

export function isGeldigeBatchovergang(
  van: Printbatchstatus,
  naar: Printbatchstatus,
): boolean {
...
export function valideerGeadresseerdeSnapshot(
  ...
  if (!snapshot.postcode.trim()) fouten.push('Postcode is verplicht.');
  if (!snapshot.plaats.trim()) fouten.push('Plaats is verplicht.');
  if (!snapshot.land.trim()) fouten.push('Land is verplicht.');
  return fouten;
}
...
export function valideerBriefcontract(brief: BriefContract): string[] {
  ...
  if (brief.actieveVersie !== null && (!Number.isInteger(brief.actieveVersie) || brief.actieveVersie < 1)) {
    fouten.push('Actieve versie moet minimaal 1 zijn.');
  }
  if (brief.status === 'definitief' && !brief.briefnummer) {
    fouten.push('Een definitieve brief vereist een briefnummer.');
  }
  if (brief.status === 'geannuleerd' && !brief.annuleringsreden?.trim()) {
    fouten.push('Een geannuleerde brief vereist een reden.');
  }
  if (brief.vergrendeldOp && brief.status === 'concept') {
    fouten.push('Een vergrendelde brief kan geen conceptstatus hebben.');
  }
  return fouten;
}

export function valideerBriefversie(versie: BriefversieContract): string[] {
  const fouten = valideerGeadresseerdeSnapshot(versie.geadresseerde);
  if (!Number.isInteger(versie.versienummer) || versie.versienummer < 1) {
    fouten.push('Versienummer moet minimaal 1 zijn.');
  }
  if (!versie.inhoud.brieftekst.trim()) fouten.push('Brieftekst is verplicht.');
  if (versie.status === 'verzonden' && !versie.verzondenOp) {
    fouten.push('Een verzonden briefversie vereist een verzenddatum.');
  }
  if (versie.status === 'vervallen' && !versie.vervallenOp) {
    fouten.push('Een vervallen briefversie vereist een vervaldatum.');
  }
  if (versie.status === 'actief' && (versie.vervallenOp || versie.verzondenOp)) {
    fouten.push('Een actieve briefversie mag niet vervallen of verzonden zijn.');
  }
  return fouten;
}
...
export function valideerPrintbatch(batch: PrintbatchContract): string[] {
  ...
  if (batch.status === 'geprint' && !batch.printdatum) {
    fouten.push('Een geprinte batch vereist een printdatum.');
  }
  if ((batch.status === 'gedeeltelijk_gepost' || batch.status === 'gepost') && !batch.verzenddatum) {
    fouten.push('Een geposte batch vereist een verzenddatum.');
  }
  if (batch.status === 'geannuleerd' && !batch.annuleringsreden?.trim()) {
    fouten.push('Een geannuleerde batch vereist een reden.');
  }
  return fouten;
}

export function magBatchinhoudWijzigen(status: Printbatchstatus): boolean {
  return status === 'concept';
}

export function magBatchdocumentenRegenereren(status: Printbatchstatus): boolean {
  return status === 'concept' || status === 'documenten_gegenereerd';
}

export function productiekernFeatureActief(envWaarde: unknown): boolean {
  return envWaarde === 'true';
}
