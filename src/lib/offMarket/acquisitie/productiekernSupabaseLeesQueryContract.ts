...
export type ProductiekernBulkLeesQueryNaam = Extract<ProductiekernLeesQueryNaam,'haal_dossiers_op_selectie_ids'|'haal_brieven_op_ids'|'haal_briefversies_op_ids'|'haal_briefversies_op_brief_ids'|'haal_printbatch_brieven_op_versie_ids'>;
export interface ProductiekernLeesQueryContract { naam:ProductiekernLeesQueryNaam; tabel:string; filterKolom:string; selectKolommen:readonly string[]; volgorde?:Readonly<{kolom:string;oplopend:boolean}>; cardinaliteit:'nul_of_een'|'lijst'; maximaalAantalRecords:number; }
export interface ProductiekernBulkLeesQueryContract extends ProductiekernLeesQueryContract { cardinaliteit:'lijst'; maximaalAantalFilterwaarden:number; }

const DOSSIER_KOLOMMEN=['selectie_id','signaal_id','vastgoedkans_id','object_id','verwerking_gestart_op','verwerking_gestart_door','primaire_werkbak','volgende_actie_op','volgende_actie_omschrijving'] as const;
const BRIEF_KOLOMMEN=['id','briefnummer','signaal_id','vastgoedkans_id','selectie_id','object_id','relatie_id','actieve_versie','status','vervanging_van_brief_id','definitief_op','vergrendeld_op','annuleringsreden'] as const;
...
const PRINTBATCH_KOLOMMEN=['id','batchnummer','bron_type','status','documentversie','aanvulling_op_batch_id','printdatum','verzenddatum','geannuleerd_op','annuleringsreden'] as const;
...
function normaliseerFilterwaarde(naam:ProductiekernLeesQueryNaam,waarde:string):string { const genormaliseerd=waarde.trim(); if(!genormaliseerd)throw new Error(`Filterwaarde voor ${naam} is verplicht.`); if(genormaliseerd.length>200)throw new Error(`Filterwaarde voor ${naam} is te lang.`); if(/[\u0000-\u001f\u007f]/u.test(genormaliseerd))throw new Error(`Filterwaarde voor ${naam} bevat controletekens.`); return genormaliseerd; }
export function bouwProductiekernLeesQuery(naam:Exclude<ProductiekernLeesQueryNaam,ProductiekernBulkLeesQueryNaam>,filterWaarde:string):ProductiekernLeesQueryContract&{filterWaarde:string}{return{...PRODUCTIEKERN_LEES_QUERY_CONTRACTEN[naam],filterWaarde:normaliseerFilterwaarde(naam,filterWaarde)}}
export function bouwProductiekernBulkLeesQuery(naam:ProductiekernBulkLeesQueryNaam,filterWaarden:readonly string[]):ProductiekernBulkLeesQueryContract&{filterWaarden:readonly string[]}{const contract=PRODUCTIEKERN_BULK_LEES_QUERY_CONTRACTEN[naam];if(filterWaarden.length===0)throw new Error(`Filterwaarden voor ${naam} zijn verplicht.`);const uniek=[...new Set(filterWaarden.map((waarde)=>normaliseerFilterwaarde(naam,waarde)))];if(uniek.length>contract.maximaalAantalFilterwaarden)throw new Error(`Te veel filterwaarden voor ${naam}.`);return{...contract,filterWaarden:Object.freeze(uniek)}}
