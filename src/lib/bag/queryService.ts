...
export function valideerPandZoekAanvraagV2(
  ...
  if (
    aanvraag.bouwjaarVan !== null
    && aanvraag.bouwjaarTot !== null
    && aanvraag.bouwjaarVan > aanvraag.bouwjaarTot
  ) fouten.push('Bouwjaar vanaf mag niet hoger zijn dan bouwjaar tot.');

  if (
    aanvraag.vboOppervlakteSomVan !== null
    && aanvraag.vboOppervlakteSomTot !== null
    && aanvraag.vboOppervlakteSomVan > aanvraag.vboOppervlakteSomTot
  ) fouten.push('VBO-oppervlakte som vanaf mag niet hoger zijn dan tot.');

  if (
    aanvraag.vboOppervlakteMaxVan !== null
    && aanvraag.vboOppervlakteMaxTot !== null
    && aanvraag.vboOppervlakteMaxVan > aanvraag.vboOppervlakteMaxTot
  ) fouten.push('VBO-oppervlakte max vanaf mag niet hoger zijn dan tot.');

  if (
    aanvraag.vboAantalVan !== null
    && aanvraag.vboAantalTot !== null
    && aanvraag.vboAantalVan > aanvraag.vboAantalTot
