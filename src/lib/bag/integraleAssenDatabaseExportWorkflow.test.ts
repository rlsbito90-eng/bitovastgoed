...
workflowDescribe('officiële Assen-export naar PostGIS-laadbestanden', () => {
  it('exporteert de volledige staginglaag zonder adapteruitval', async () => {
    ...
    expect(resultaat.geometrieen + resultaat.overgeslagenGeometrieen).toBe(122_388);

    for (const bestand of ['objecten.csv', 'voorkomens.csv', 'relaties.csv', 'geometrieen.csv', 'manifest.json']) {
