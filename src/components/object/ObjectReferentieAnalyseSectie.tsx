...
export default function ObjectReferentieAnalyseSectie({ object }: Props) {
  const store = useDataStore();
  const gekoppeld = store.getReferentiesVoorObject(object.id);
  const koppelingen = store.objectReferenties.filter(x => x.objectId === object.id);
  ...
  // AUTO-SUGGESTIES op basis van object-eigenschappen
  ...
  }, [object, store.referentieObjecten, gekoppeld]);
  ...
  const handleKoppel = async (ref: ReferentieObject) => {
    try {
      await store.koppelReferentieAanObject(object.id, ref.id);
  ...
  const handleOntkoppel = async (refId: string) => {
    ...
    if (!koppeling) return;
    try {
      await store.ontkoppelReferentieVanObject(koppeling.id);
  ...
  const handleNieuwAangemaakt = async (saved: ReferentieObject) => {
    try {
      await store.koppelReferentieAanObject(object.id, saved.id);
