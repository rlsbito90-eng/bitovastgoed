...
export default function GeadresseerdenLijst({geadresseerden}:GeadresseerdenLijstProps){
 ...
 return <section className="mt-2 rounded-md border border-border/70 bg-muted/20 px-2.5 py-2" data-testid="acquisitie-rij-geadresseerden" aria-label={`${geadresseerden.length} eigenaar/geadresseerde${geadresseerden.length===1?'':'n'}`}>
  ...
  <ul className="space-y-1.5 text-[11px] text-muted-foreground">{geadresseerden.map((g)=>{ const adres=weergaveadresGeadresseerde(g.verzendadres); const isEmail=isEmailContactwaarde(adres); const key=partijKeyVoorKandidaat(g); const partij=key?partijOverzicht.perKey.get(key):undefined; const bekendePartij=partij&&partij.objecten.length>=2?partij:undefined; return <li key={g.key} data-testid="acquisitie-rij-geadresseerde" className="min-w-0 break-words">
   <div className="flex flex-wrap items-center gap-1.5"><div className="font-medium text-foreground" data-testid="acquisitie-rij-geadresseerde-naam">{weergavenaamGeadresseerde(g)}</div>{bekendePartij&&<button type="button" data-testid="acquisitie-rij-bekende-partij" onClick={()=>openAcquisitiePartij(bekendePartij.key)} className="inline-flex items-center gap-1 rounded-full border border-amber-300 bg-amber-50 px-1.5 py-0.5 text-[10px] font-semibold text-amber-900 hover:bg-amber-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" title={`Open ${bekendePartij.naam} in Eigenaren & portefeuilles`}><Layers3 className="h-3 w-3"/>Bekende partij · {bekendePartij.objecten.length} objecten</button>}</div>
   {adres?(isEmail?<div data-testid="acquisitie-rij-geadresseerde-email"><span className="text-muted-foreground/80">E-mail:</span> {adres}</div>:<div data-testid="acquisitie-rij-geadresseerde-adres"><span className="text-muted-foreground/80">Postadres:</span> {adres}{!g.volledigPostadres&&<span className="text-destructive"> · adres onvolledig</span>}</div>):<div className="text-destructive" data-testid="acquisitie-rij-geadresseerde-adres-ontbreekt">Postadres ontbreekt</div>}
