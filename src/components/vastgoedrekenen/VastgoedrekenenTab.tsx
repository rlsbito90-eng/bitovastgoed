...
function QuickscanDetail({ calculationId, taxSettings, objectArea, objectWoz, objectEnergyLabel, objectBouwjaar, viewMode, objectRawType, objectVraagprijs }: {
  ...
  const safeScenarios = scenarios.filter((scenario) => Boolean(scenario?.id));
  ...
  if (!calculation?.id) {
    return <p className="text-sm text-muted-foreground">Quickscan wordt geladen…</p>;
  }
        ...
        <div className="border-b border-border/60 bg-gradient-to-r from-primary/[0.07] via-background to-accent/[0.05] px-4 py-4 sm:px-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              <h2 className="mt-2 truncate text-xl font-semibold tracking-tight text-foreground sm:text-2xl">{calculation.calculation_name}</h2>
              <p className="mt-1 text-sm text-muted-foreground">
                {VR_STRATEGY_LABELS[calculation.main_strategy]} · {calculation.object_type === 'mixed_use' ? 'Mixed-use' : 'Enkelvoudig'} · {safeScenarios.length} scenario{safeScenarios.length === 1 ? '' : "'s"}
              </p>
            </div>
            <Button onClick={createAndOpenScenario} className="shrink-0 shadow-sm"><Plus className="mr-1.5 h-4 w-4" /> Nieuw scenario</Button>
          </div>
        </div>
        <div className="overflow-x-auto px-2 py-2 sm:px-3">
          <TabsList className="inline-flex h-auto min-w-max gap-1 bg-transparent p-0">
            {CASE_WORKSPACE_TABS.map((tab) => {
              const Icon = tab.icon;
              return (
                <TabsTrigger key={tab.value} value={tab.value} className="group min-h-[52px] min-w-[150px] justify-start gap-2 rounded-lg border border-transparent px-3 py-2 text-left data-[state=active]:border-border data-[state=active]:bg-card data-[state=active]:shadow-sm">
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
          ...
          <Card className="border-border/60 bg-muted/20 shadow-sm">
            ...
            <CardContent className="space-y-3">
              <p className="text-sm text-muted-foreground">Werk de objectuitgangspunten bij of open direct een scenario om de businesscase door te rekenen.</p>
              <Button variant="outline" className="w-full justify-between" onClick={() => setWorkspaceTab('scope')}>Object & uitgangspunten <ChevronRight className="h-4 w-4" /></Button>
              <Button className="w-full justify-between" onClick={() => setWorkspaceTab('scenarios')}>Naar scenario’s <ChevronRight className="h-4 w-4" /></Button>
            </CardContent>
          </Card>
        </div>
      </TabsContent>
        ...
        {safeScenarios.length === 0 ? (
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center py-14 text-center">
              ...
              <Button className="mt-4" onClick={createAndOpenScenario}><Plus className="mr-1.5 h-4 w-4" /> Eerste scenario maken</Button>
            </CardContent>
          </Card>
        ) : (
            ...
            <Card className="h-fit border-border/60 shadow-sm xl:sticky xl:top-[168px]">
              <CardHeader className="border-b border-border/50 pb-3">
                <div className="flex items-center justify-between gap-3">
                  <div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-accent">Scenario’s</p><CardTitle className="mt-1 text-base">Selecteer werkblad</CardTitle></div>
                  <Button size="icon" variant="outline" onClick={createAndOpenScenario} title="Nieuw scenario"><Plus className="h-4 w-4" /></Button>
                </div>
              </CardHeader>
              <CardContent className="space-y-2 p-2">
                {safeScenarios.map((scenario) => {
                  ...
                  return (
                    <button key={scenario.id} type="button" onClick={() => setActiveScenarioId(scenario.id)} className={`w-full rounded-lg border p-3 text-left transition-all ${selected ? 'border-primary/35 bg-primary/[0.07] shadow-sm' : 'border-transparent bg-muted/30 hover:border-border hover:bg-muted/60'}`}>
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <p className="truncate text-sm font-semibold">{scenario.scenario_name || 'Naamloos scenario'}</p>
                          <p className="mt-1 line-clamp-2 text-[11px] leading-snug text-muted-foreground">{getBusinessCaseLabel(taxonomy.value.businessCase)} · {getInterventionLabel(taxonomy.value.intervention)}</p>
                        </div>
                ...
                <CardHeader className="border-b border-border/50 bg-gradient-to-r from-muted/50 to-background pb-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                      ...
                      <CardTitle className="mt-1 truncate text-lg">{activeScenario.scenario_name || 'Naamloos scenario'}</CardTitle>
                    ...
                    <Button type="button" variant="outline" size="sm" disabled={duplicatingId === activeScenario.id} onClick={() => duplicateAndOpen(activeScenario.id)}>
                      <Copy className="mr-1.5 h-4 w-4" />{duplicatingId === activeScenario.id ? 'Kopiëren…' : 'Dupliceren'}
                    </Button>
                  </div>
                </CardHeader>
                <CardContent className="p-3 sm:p-5">
                  <Tabs defaultValue="setup" className="space-y-5">
                    <div className="overflow-x-auto rounded-lg border border-border/60 bg-muted/30 p-1">
                      <TabsList className="inline-flex h-auto min-w-max gap-1 bg-transparent p-0">
                        <TabsTrigger value="setup" className="gap-2 px-3 py-2"><FileSpreadsheet className="h-4 w-4" /> Opzet & classificatie</TabsTrigger>
                        <TabsTrigger value="assumptions" className="gap-2 px-3 py-2"><SlidersHorizontal className="h-4 w-4" /> Kengetallen & aannames</TabsTrigger>
                        <TabsTrigger value="calculation" className="gap-2 px-3 py-2"><BarChart3 className="h-4 w-4" /> Doorrekenen</TabsTrigger>
                      </TabsList>
                    </div>
                    <TabsContent value="setup" className="mt-0 space-y-4">
                      <ScenarioTaxonomyPanel scenario={activeScenario} onSave={(patch) => saveScenarioTaxonomy(activeScenario.id, patch)} onSyncCompatibility={(patch) => syncScenarioCompatibility(activeScenario.id, patch)} />
                      {proposition.propositionType === 'renovate_and_sell' && <RenovateAndSellPanel scenario={activeScenario} onSaved={refetch} />}
                    </TabsContent>
                    ...
                    <TabsContent value="calculation" className="mt-0">
                      <ScenarioEditor scenario={activeScenario} taxSettings={taxSettings} objectType={calculation.object_type} objectArea={objectArea} objectWoz={objectWoz} objectEnergyLabel={objectEnergyLabel} objectBouwjaar={objectBouwjaar} objectRawType={objectRawType} objectVraagprijs={objectVraagprijs} viewMode={viewMode} onUpdate={updateScenario} onDelete={deleteScenario} />
                    </TabsContent>
                  </Tabs>
                </CardContent>
              </Card>
...
export default function VastgoedrekenenTab({ objectId, objectArea, objectWoz, objectEnergyLabel, objectBouwjaar, objectRawType, objectVraagprijs, initialCalculationId }: Props) {
  ...
  const safeCalculations = calculations.filter((calculation) => Boolean(calculation?.id));
  ...
  useEffect(() => {
    ...
    setActiveId((current) => (
      current && safeCalculations.some((calculation) => calculation.id === current)
        ? current
        : safeCalculations[0]?.id ?? null
    ));
  }, [safeCalculations, initialCalculationId]);

  const active = activeId && safeCalculations.some((calculation) => calculation.id === activeId)
    ? activeId
    : safeCalculations[0]?.id ?? null;
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
              <Select value={viewMode} onValueChange={(v) => setViewMode(v as 'begeleid' | 'compact' | 'expert')}>
                ...
                <SelectContent><SelectItem value="begeleid">Begeleid</SelectItem><SelectItem value="compact">Compact</SelectItem><SelectItem value="expert">Expert</SelectItem></SelectContent>
              </Select>
              <Button className="w-full shadow-sm sm:w-auto" onClick={() => setCreateOpen(true)}><Plus className="mr-1.5 h-4 w-4" /> Nieuwe analyse</Button>
        ...
        {safeCalculations.length > 0 && (
          <CardContent className="flex gap-2 overflow-x-auto p-3">
            {safeCalculations.map((calculation) => (
              <button key={calculation.id} onClick={() => selectQuickscan(calculation.id)} className={`min-w-[190px] rounded-lg border px-3 py-2.5 text-left transition-all ${active === calculation.id ? 'border-primary/35 bg-primary/[0.07] shadow-sm' : 'border-border/50 bg-card hover:border-primary/25 hover:bg-muted/30'}`}>
                <p className="truncate text-sm font-semibold">{calculation.calculation_name || 'Naamloze analyse'}</p>
                <div className="mt-1 flex items-center gap-1.5 text-[10px] text-muted-foreground"><span>{VR_STATUS_LABELS[calculation.status]}</span><span>·</span><span className="truncate">{VR_STRATEGY_LABELS[calculation.main_strategy]}</span></div>
              </button>
            ))}
          </CardContent>
        )}
      ...
      {active ? (
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-14 text-center">
            ...
            <Button className="mt-4" onClick={() => setCreateOpen(true)}><Plus className="mr-1.5 h-4 w-4" /> Nieuwe analyse</Button>
          </CardContent>
        </Card>
      )}
    </div>
