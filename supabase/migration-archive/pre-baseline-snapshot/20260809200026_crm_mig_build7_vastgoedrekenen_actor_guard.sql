...
begin
  ...
  if new.status='goedgekeurd' and (tg_op='INSERT' or old.status is distinct from 'goedgekeurd') and not new.system_managed then
...
create trigger vastgoedrekenen_bronpakket_actor_guard before insert or update on public.vastgoedrekenen_bronpakketten for each row execute function public.vastgoedrekenen_enforce_bronpakket_actor();