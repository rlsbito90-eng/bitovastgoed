-- TRACK-2 release 4 — reconciliatie van reeds toegepaste productiemigratie.
...
declare v_briefnummer text; v_status text; v_bestaand_nummer text; v_versienummer integer;
begin
  ...
  select b.status,b.briefnummer into v_status,v_bestaand_nummer from public.off_market_brieven b where b.id=p_brief_id for update;
  ...
  values(gen_random_uuid(),p_operation_key,'briefnummer_uitgegeven',p_brief_id,p_brief_versie_id,p_actor_id,p_uitgevoerd_op,jsonb_build_object('briefnummer',v_briefnummer,'versienummer',v_versienummer));
  return query select p_brief_id,v_briefnummer;
end; $$;

create or replace function public.off_market_batch_documenten_registreren(
  p_batch_id uuid,p_actor_id uuid,p_operation_key text,p_verwacht_documentversie integer,p_uitgevoerd_op timestamptz,p_documenten jsonb
)
returns void language plpgsql security definer set search_path=public,pg_temp as $$
declare v_status text; v_documentversie integer; v_document jsonb; v_typen text[]:=array[]::text[];
begin
  if jsonb_typeof(p_documenten)<>'array' or jsonb_array_length(p_documenten)<>4 then raise exception 'exact_vier_batchdocumenten_verplicht'; end if;
  perform pg_advisory_xact_lock(hashtextextended(p_operation_key,0));
  if exists(select 1 from public.off_market_productie_events where operation_key=p_operation_key) then return; end if;
  select status,documentversie into v_status,v_documentversie from public.off_market_printbatches where id=p_batch_id for update;
  if not found then raise exception 'batch_niet_gevonden'; end if;
  if v_status not in ('concept','documenten_gegenereerd') then raise exception 'batchstatus_blokkeert_documentregistratie'; end if;
  ...
  for v_document in select value from jsonb_array_elements(p_documenten) loop
    if (v_document->>'documenttype') not in ('brieven_pdf','adreslabels','controlelijst','batchvoorblad') then raise exception 'ongeldig_documenttype'; end if;
    if nullif(trim(v_document->>'bestand_referentie'),'') is null then raise exception 'bestand_referentie_verplicht'; end if;
    v_typen:=array_append(v_typen,v_document->>'documenttype');
  end loop;
  if cardinality(array(select distinct unnest(v_typen)))<>4 then raise exception 'ieder_documenttype_exact_een_keer_verplicht'; end if;
  update public.off_market_batchdocumenten set status='vervallen',vervallen_op=p_uitgevoerd_op where batch_id=p_batch_id and status='actief';
  insert into public.off_market_batchdocumenten(id,batch_id,documentversie,documenttype,bestand_referentie,status,metadata,aangemaakt_door,created_at)
  select gen_random_uuid(),p_batch_id,p_verwacht_documentversie,d->>'documenttype',d->>'bestand_referentie','actief',coalesce(d->'metadata','{}'::jsonb),p_actor_id,p_uitgevoerd_op
  from jsonb_array_elements(p_documenten) d;
  update public.off_market_printbatches set status='documenten_gegenereerd' where id=p_batch_id;
  insert into public.off_market_productie_events(id,operation_key,event_type,batch_id,actor_id,event_at,metadata)
  values(gen_random_uuid(),p_operation_key,'documenten_gegenereerd',p_batch_id,p_actor_id,p_uitgevoerd_op,jsonb_build_object('documentversie',p_verwacht_documentversie));
end; $$;
...
begin
  ...
  if v_documentversie<>p_verwacht_documentversie then raise exception 'optimistic_lock_conflict'; end if;
  update public.off_market_printbatches set status='geprint',printdatum=p_printdatum where id=p_batch_id;
  insert into public.off_market_productie_events(id,operation_key,event_type,batch_id,actor_id,event_at,metadata)
