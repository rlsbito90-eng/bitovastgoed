-- TRACK-2 release 3 — reconciliatie van reeds toegepaste productiemigratie.
...
begin
  ...
  if exists(select 1 from public.off_market_productie_events where operation_key=p_operation_key) then
    return query select e.selectie_id,e.signaal_id from public.off_market_productie_events e where e.operation_key=p_operation_key; return;
  end if;
  select s.signaal_id into v_signaal_id from public.off_market_acquisitie_selectie s where s.id=p_selectie_id and s.archived_at is null;
  if not found then raise exception 'selectie_niet_gevonden'; end if;
  insert into public.off_market_acquisitie_dossiers(id,selectie_id,signaal_id,verwerking_gestart_op,verwerking_gestart_door,primaire_werkbak,created_at,updated_at)
  values(gen_random_uuid(),p_selectie_id,v_signaal_id,p_uitgevoerd_op,p_actor_id,'eigenaar_achterhalen',p_uitgevoerd_op,p_uitgevoerd_op)
  ...
  insert into public.off_market_productie_events(id,operation_key,event_type,signaal_id,selectie_id,actor_id,event_at)
  values(gen_random_uuid(),p_operation_key,'verwerking_gestart',v_signaal_id,p_selectie_id,p_actor_id,p_uitgevoerd_op);
...
language plpgsql security definer set search_path=public,pg_temp as $$
declare v_signaal_id uuid; v_brief_id uuid;
begin
  if nullif(trim(p_operation_key),'') is null then raise exception 'operation_key_verplicht'; end if;
  perform pg_advisory_xact_lock(hashtextextended(p_operation_key,0));
  if exists(select 1 from public.off_market_productie_events where operation_key=p_operation_key) then
    return query select e.brief_id,e.signaal_id from public.off_market_productie_events e where e.operation_key=p_operation_key; return;
  end if;
  select d.signaal_id into v_signaal_id from public.off_market_acquisitie_dossiers d where d.selectie_id=p_selectie_id and d.verwerking_gestart_op is not null for update;
  ...
  update public.off_market_acquisitie_dossiers set primaire_werkbak='brief_opstellen',updated_at=p_uitgevoerd_op where selectie_id=p_selectie_id;
  insert into public.off_market_productie_events(id,operation_key,event_type,signaal_id,selectie_id,brief_id,actor_id,event_at)
  values(gen_random_uuid(),p_operation_key,'brief_aangemaakt',v_signaal_id,p_selectie_id,v_brief_id,p_actor_id,p_uitgevoerd_op);
...
language plpgsql security definer set search_path=public,pg_temp as $$
declare v_status text; v_volgend integer; v_versie_id uuid;
begin
  if nullif(trim(p_operation_key),'') is null then raise exception 'operation_key_verplicht'; end if;
  ...
  perform pg_advisory_xact_lock(hashtextextended(p_operation_key,0));
  if exists(select 1 from public.off_market_productie_events where operation_key=p_operation_key) then
    return query select e.brief_versie_id,(e.metadata->>'versienummer')::integer from public.off_market_productie_events e where e.operation_key=p_operation_key; return;
  end if;
  ...
  update public.off_market_brief_versies set status='vervallen',vervallen_op=p_uitgevoerd_op where brief_id=p_brief_id and status='actief';
  v_versie_id:=gen_random_uuid();
  insert into public.off_market_brief_versies(id,brief_id,versienummer,status,inhoud_snapshot,geadresseerde_snapshot,aangemaakt_door,created_at)
  values(v_versie_id,p_brief_id,v_volgend,'actief',p_inhoud_snapshot,p_geadresseerde_snapshot,p_actor_id,p_uitgevoerd_op);
  update public.off_market_brieven set
    actieve_versie=v_volgend,
  ...
  where id=p_brief_id;
  insert into public.off_market_productie_events(id,operation_key,event_type,brief_id,brief_versie_id,actor_id,event_at,metadata)
  values(gen_random_uuid(),p_operation_key,'briefversie_aangemaakt',p_brief_id,v_versie_id,p_actor_id,p_uitgevoerd_op,jsonb_build_object('versienummer',v_volgend));
...
language plpgsql security definer set search_path=public,pg_temp as $$
declare v_batch_id uuid; v_batchnummer text;
begin
  if nullif(trim(p_operation_key),'') is null then raise exception 'operation_key_verplicht'; end if;
  perform pg_advisory_xact_lock(hashtextextended(p_operation_key,0));
  if exists(select 1 from public.off_market_productie_events where operation_key=p_operation_key) then
    return query select e.batch_id,e.metadata->>'batchnummer' from public.off_market_productie_events e where e.operation_key=p_operation_key; return;
  end if;
  v_batchnummer:=public.reserveer_off_market_batchnummer(p_datum); v_batch_id:=gen_random_uuid();
  ...
  values(gen_random_uuid(),p_operation_key,'batchnummer_uitgegeven',v_batch_id,p_actor_id,p_uitgevoerd_op,jsonb_build_object('batchnummer',v_batchnummer));
  return query select v_batch_id,v_batchnummer;
end; $$;

create or replace function public.off_market_briefversie_aan_batch_toevoegen(
  p_batch_id uuid,p_brief_id uuid,p_brief_versie_id uuid,p_actor_id uuid,p_operation_key text,p_uitgevoerd_op timestamptz
)
...
begin
  ...
  if not exists(select 1 from public.off_market_brief_versies where id=p_brief_versie_id and brief_id=p_brief_id and status='actief') then raise exception 'actieve_briefversie_niet_gevonden'; end if;
  ...
  insert into public.off_market_productie_events(id,operation_key,event_type,brief_id,brief_versie_id,batch_id,actor_id,event_at)
  values(gen_random_uuid(),p_operation_key,'brief_aan_batch_toegevoegd',p_brief_id,p_brief_versie_id,p_batch_id,p_actor_id,p_uitgevoerd_op);
end; $$;
