-- TRACK-2 release 5 — reconciliatie van reeds toegepaste productiemigratie.
...
begin
  ...
  if not exists (
    ...
    where batch_id = p_batch_id and brief_id = p_brief_id and brief_versie_id = p_brief_versie_id and verwijderd_op is null
  ) then raise exception 'briefversie_niet_in_batch'; end if;

  if not exists (
    select 1 from public.off_market_acquisitie_dossiers where selectie_id = v_selectie_id for update
  ) then raise exception 'acquisitiedossier_niet_gevonden'; end if;
  ...
  insert into public.off_market_productie_events (
    ...
    jsonb_build_object('geadresseerde_key', p_geadresseerde_key, 'opvolgdatum', p_verzenddatum + interval '14 days')
  );
  ...
  where pb.batch_id = p_batch_id and pb.verwijderd_op is null and bv.status <> 'verzonden';
