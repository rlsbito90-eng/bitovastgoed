...
function addressCandidates(lines: string[]): AddressCandidate[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // One-line address: "Straat 12 1234 AB Plaats".
...
export function parseKadasterPdfOwnerAddresses(text: string, hints: PdfOwnerHint[]): PdfOwnerAdresMatch[] {
  ...
  for (const hint of hints) {
    ...
    // The same address may legitimately belong to multiple owners, but do not let
    // a weak surname-only match steal an address already assigned earlier.
