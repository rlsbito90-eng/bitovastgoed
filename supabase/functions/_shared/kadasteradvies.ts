// 1-op-1 server-port van src/lib/offMarket/bag/kadasteradvies.ts.
// Pure helper, geen IO. Houd in sync met de client-versie.
//
// Wordt door off-market-bag-verrijk aangeroepen nadat bag_status='verrijkt'
// is gepersisteerd. Roept GEEN Kadaster-API aan.

import { strategieMatchVoorBag, type SignaalAutoInput } from './offMarketAutoTrigger.ts';

export type Kadasteradvies = 'laag' | 'voorzichtig' | 'aanbevolen' | 'sterk_aanbevolen';

export interface SignaalKadasterInput extends SignaalAutoInput {
...
function strategieSterk(s: SignaalKadasterInput): boolean {
  const blob = [
...
function strategieKamerverhuur(s: SignaalKadasterInput): boolean {
  const blob = [
    s.ai_strategie_suggestie ?? '',
    s.potentiele_strategie ?? '',
  ].join(' ').toLowerCase();
...
export function berekenKadasteradvies(s: SignaalKadasterInput): KadasteradviesResultaat {
