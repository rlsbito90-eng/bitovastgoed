...
begin
  for v_jobid in select jobid from cron.job where jobname in ('bito-notification-engine-5m','bito-notification-push-1m') loop
    perform cron.unschedule(v_jobid);
  end loop;
  perform cron.schedule('bito-notification-engine-5m','*/5 * * * *','select public.notification_engine_http_tick();');
  perform cron.schedule('bito-notification-push-1m','* * * * *','select public.notification_push_http_tick();');
end $$;
