...
SET proposition_type = COALESCE(proposition_type, 'legacy_generic'),
    proposition_schema_version = COALESCE(proposition_schema_version, 1)
WHERE proposition_type IS NULL
   OR proposition_schema_version IS NULL;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'real_estate_calculations_proposition_type_check'
  ) THEN
    ALTER TABLE public.real_estate_calculations
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'real_estate_calculations_proposition_schema_version_check'
  ) THEN
    ALTER TABLE public.real_estate_calculations
