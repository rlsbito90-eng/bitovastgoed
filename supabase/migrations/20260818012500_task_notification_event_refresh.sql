-- Bito CRM — server-side afleiding van taaknotificaties.
-- Geen delivery/push hier: deze functie onderhoudt uitsluitend de logische events.
...
begin
  ...
  -- Vandaag: één logisch occurrence-event per taak per datum.
  ...
  select
    ...
    'taak',
    t.id::text,
    'task_due_today:' || t.id::text || ':' || v_today::text,
  ...
  -- Verlopen: één event per taak voor de huidige overdue episode. Een gewijzigde
  -- deadline krijgt een nieuwe occurrence-key; oude toestand wordt hieronder resolved.
  insert into public.notification_events (
    user_id, event_type, source_type, source_id, occurrence_key,
    title, body, priority, href, metadata
  )
  select
    ...
    'taak',
    t.id::text,
    'task_overdue:' || t.id::text || ':' || coalesce(t.deadline::text, 'no-date') || ':' || coalesce(left(t.deadline_tijd::text, 5), 'eod'),
    'Taak verlopen',
    t.titel,
    'kritiek',
    '/taken/' || t.id::text,
  ...
  where t.owner_user_id = p_user_id
    ...
    and (
      ...
      or (
        ...
        and (t.deadline + t.deadline_tijd) < v_now_local
      )
    )
  on conflict (user_id, occurrence_key) do nothing;
  ...
  -- Hoge/urgente taak: één event per taak, ongeacht datum.
  insert into public.notification_events (
    user_id, event_type, source_type, source_id, occurrence_key,
    title, body, priority, href, metadata
  )
  ...
  -- Resolve alle taak-events waarvan de bron niet langer actief/relevant is.
...
comment on function public.refresh_task_notification_events(uuid) is
  'Idempotente server-side projectie van taakstatus naar gebruikersbrede notification_events.';
