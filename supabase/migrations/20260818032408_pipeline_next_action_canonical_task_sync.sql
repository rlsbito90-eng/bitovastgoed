-- Object Pipeline volgende actie -> centrale taak.
-- Bezichtiging en gewenste levering blijven bewust afspraak/prognose en worden hier NIET geraakt.
...
begin
  v_active := new.soft_deleted_at is null
    and new.pipeline_fase not in ('afgerond', 'afgevallen', 'afgehaakt', 'afgewezen_door_ons', 'gewonnen')
...
drop trigger if exists trg_sync_pipeline_next_action_task on public.object_pipeline;
create trigger trg_sync_pipeline_next_action_task
after insert or update of volgende_actie, volgende_actie_omschrijving, volgende_actie_datum, pipeline_fase, soft_deleted_at, relatie_id, object_id
on public.object_pipeline
for each row
execute function public.sync_pipeline_next_action_task();
