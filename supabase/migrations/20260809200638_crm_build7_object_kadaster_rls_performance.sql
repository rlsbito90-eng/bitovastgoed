create index if not exists crm_objectregistraties_samengevoegd_in_idx on public.crm_objectregistraties (samengevoegd_in_id) where samengevoegd_in_id is not null;
...
create policy "crm_objectregistraties_toevoegen" on public.crm_objectregistraties for insert to authenticated with check (created_by = (select auth.uid()) or (select public.is_app_admin()));
drop policy if exists "crm_objectregistraties_bijwerken" on public.crm_objectregistraties;
create policy "crm_objectregistraties_bijwerken" on public.crm_objectregistraties for update to authenticated using (created_by = (select auth.uid()) or (select public.is_app_admin())) with check (created_by = (select auth.uid()) or (select public.is_app_admin()));
drop policy if exists "crm_objectregistraties_beheerder_verwijderen" on public.crm_objectregistraties;
create policy "crm_objectregistraties_beheerder_verwijderen" on public.crm_objectregistraties for delete to authenticated using ((select public.is_app_admin()));

drop policy if exists "crm_objectbronkoppelingen_toevoegen" on public.crm_objectbronkoppelingen;
create policy "crm_objectbronkoppelingen_toevoegen" on public.crm_objectbronkoppelingen for insert to authenticated with check (created_by = (select auth.uid()) or (select public.is_app_admin()));
drop policy if exists "crm_objectbronkoppelingen_bijwerken" on public.crm_objectbronkoppelingen;
create policy "crm_objectbronkoppelingen_bijwerken" on public.crm_objectbronkoppelingen for update to authenticated using (created_by = (select auth.uid()) or (select public.is_app_admin())) with check (created_by = (select auth.uid()) or (select public.is_app_admin()));
drop policy if exists "crm_objectbronkoppelingen_beheerder_verwijderen" on public.crm_objectbronkoppelingen;
create policy "crm_objectbronkoppelingen_beheerder_verwijderen" on public.crm_objectbronkoppelingen for delete to authenticated using ((select public.is_app_admin()));
