...
begin
  ...
  -- Deadline vandaag.
  -- Zonder tijd: dagmelding zodra de dag actief is.
  -- Met tijd: pas vanaf het expliciete lokale deadline-tijdstip.
  ...
  -- Overdue-notificatie is een volgende-dag/escalatie-signaal, niet een tweede
  -- push enkele minuten na een timed deadline. De gewone taak-UI mag een taak
  -- uiteraard direct na het tijdstip al als verlopen tonen.
  insert into public.notification_events (
    user_id, event_type, source_type, source_id, occurrence_key,
    title, body, priority, href, metadata
  )
...
-- Bestaande actieve 'vandaag'-events krijgen de nieuwe tijdspecifieke occurrence-key,
-- zodat reeds verzonden events niet nogmaals worden aangemaakt na deze migratie.
do $$
declare
  v_today date := (now() at time zone 'Europe/Amsterdam')::date;
begin
  ...
  -- Ruim dezelfde-dag overdue-events op die onder de oude regel al waren ontstaan.
  update public.notification_events e
     set resolved_at = now(), updated_at = now()
    from public.taken t
   where e.user_id = t.owner_user_id
