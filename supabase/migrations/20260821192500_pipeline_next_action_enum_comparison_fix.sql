-- Fix enum/runtime drift in object pipeline task synchronization.
-- `volgende_actie` is a Postgres enum. Comparing it directly against legacy
-- string literals that are not enum members makes PostgreSQL try to cast
-- those literals to the enum and abort the update. Compare as text instead.
...
begin
  ...
  -- Cast the enum expression, not the literals. This keeps legacy aliases
  -- harmless while canonical enum values continue to work normally.
  v_type := case new.volgende_actie::text
