-- BUILD A — PRODUCTIEKERN SECURITY WRAPPERS
-- NIET AUTOMATISCH TOEPASSEN.
-- Reviewdraft buiten supabase/migrations. Deze draft voegt geen client-grants toe
-- en eindigt expliciet met ROLLBACK.
--
-- Doel:
-- 1. behoud de reeds bewezen transactionele implementaties als interne functies;
-- 2. laat de publieke RPC-namen eerst auth.uid(), interne CRM-rol en actor-id toetsen;
-- 3. voorkom dat een authenticated gebruiker audit-events namens een ander schrijft.

begin;

create or replace function public.off_market_productiekern_assert_interne_actor(
  p_actor_id uuid
)
returns void
language plpgsql
...
begin
  ...
  if v_user_id is null then
    raise exception 'productiekern_auth_verplicht';
  end if;

  if p_actor_id is null or p_actor_id is distinct from v_user_id then
    raise exception 'productiekern_actor_mismatch';
  end if;

  if not public.is_intern_gebruiker(v_user_id) then
    raise exception 'productiekern_intern_gebruiker_verplicht';
  end if;
end;
$$;

revoke all on function public.off_market_productiekern_assert_interne_actor(uuid)
  from public, anon, authenticated;

-- De negen eerder bewezen RPC-implementaties worden intern gemaakt. Hun
-- function bodies blijven ongewijzigd; alleen de publieke toegangslaag wijzigt.
alter function public.off_market_verwerking_starten(uuid,uuid,text,timestamptz)
  rename to off_market_verwerking_starten_intern;
alter function public.off_market_brief_reserveren(uuid,uuid,text,timestamptz)
  rename to off_market_brief_reserveren_intern;
alter function public.off_market_briefversie_aanmaken(uuid,uuid,text,timestamptz,jsonb,jsonb)
  rename to off_market_briefversie_aanmaken_intern;
alter function public.off_market_printbatch_aanmaken(uuid,text,timestamptz,date)
  rename to off_market_printbatch_aanmaken_intern;
alter function public.off_market_briefversie_aan_batch_toevoegen(uuid,uuid,uuid,uuid,text,timestamptz)
  rename to off_market_briefversie_aan_batch_toevoegen_intern;

alter function public.off_market_brief_definitief_maken(uuid,uuid,uuid,text,integer,timestamptz,integer)
  rename to off_market_brief_definitief_maken_intern;
alter function public.off_market_batch_documenten_registreren(uuid,uuid,text,integer,timestamptz,jsonb)
  rename to off_market_batch_documenten_registreren_intern;
alter function public.off_market_batch_geprint_markeren(uuid,uuid,text,integer,timestamptz)
  rename to off_market_batch_geprint_markeren_intern;
alter function public.off_market_brief_gepost_markeren(uuid,uuid,uuid,text,uuid,text,integer,timestamptz)
  rename to off_market_brief_gepost_markeren_intern;

-- Interne implementaties zijn nooit rechtstreeks client-callable.
...
create function public.off_market_verwerking_starten(
  p_selectie_id uuid, p_actor_id uuid, p_operation_key text, p_uitgevoerd_op timestamptz
)
...
create function public.off_market_brief_reserveren(
  p_selectie_id uuid, p_actor_id uuid, p_operation_key text, p_uitgevoerd_op timestamptz
)
...
create function public.off_market_briefversie_aanmaken(
  p_brief_id uuid, p_actor_id uuid, p_operation_key text, p_uitgevoerd_op timestamptz,
  p_inhoud_snapshot jsonb, p_geadresseerde_snapshot jsonb
)
...
create function public.off_market_printbatch_aanmaken(
  p_actor_id uuid, p_operation_key text, p_uitgevoerd_op timestamptz, p_datum date
)
...
create function public.off_market_briefversie_aan_batch_toevoegen(
  p_batch_id uuid, p_brief_id uuid, p_brief_versie_id uuid, p_actor_id uuid,
  p_operation_key text, p_uitgevoerd_op timestamptz
)
returns void
language plpgsql security definer set search_path=public,pg_temp
as $$ begin
...
create function public.off_market_brief_definitief_maken(
  p_brief_id uuid, p_brief_versie_id uuid, p_actor_id uuid, p_operation_key text,
  p_verwacht_versienummer integer, p_uitgevoerd_op timestamptz, p_jaar integer
)
...
create function public.off_market_batch_documenten_registreren(
  p_batch_id uuid, p_actor_id uuid, p_operation_key text,
  p_verwacht_documentversie integer, p_uitgevoerd_op timestamptz, p_documenten jsonb
)
returns void
language plpgsql security definer set search_path=public,pg_temp
as $$ begin
...
create function public.off_market_batch_geprint_markeren(
  p_batch_id uuid, p_actor_id uuid, p_operation_key text,
  p_verwacht_documentversie integer, p_printdatum timestamptz
)
returns void
language plpgsql security definer set search_path=public,pg_temp
as $$ begin
...
create function public.off_market_brief_gepost_markeren(
  p_brief_id uuid, p_brief_versie_id uuid, p_batch_id uuid, p_geadresseerde_key text,
  p_actor_id uuid, p_operation_key text, p_verwacht_versienummer integer, p_verzenddatum timestamptz
)
returns void
language plpgsql security definer set search_path=public,pg_temp
as $$ begin
...
-- Ook de publieke wrappers blijven standaard dicht. Een afzonderlijke,
-- expliciet goedgekeurde activatiemigratie mag later uitsluitend EXECUTE op
-- deze negen wrappers aan authenticated geven; nooit op *_intern of de helper.
